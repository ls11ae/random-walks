...
void tensor_flat(const Tensor *t, float *values) {
    ...
    uint32_t mat_len = t->data[0]->width * t->data[0]->height;
    for (uint32_t i = 0; i < t->len; ++i) {
        memcpy(values + i * mat_len, t->data[i]->data, mat_len * sizeof(float));
    }
}
...
Tensor *tensor_from_flat(const float *flat, uint32_t tensor_len, int32_t mat_width, int32_t mat_height) {
    ...
    uint32_t mat_len = mat_width * mat_height;

    for (uint32_t i = 0; i < tensor_len; ++i) {
        t->data[i] = matrix_new(mat_width, mat_height);
        if (!t->data[i]) {
            tensor_free(t); // Hilfsfunktion zum Aufräumen
            return NULL;
        }
        memcpy(t->data[i]->data, flat + i * mat_len, mat_len * sizeof(float));
    }
...
void dir_kernel_to_cuda(const Vector2D *input, int2 **out_offsets, int **out_sizes, uint32_t *out_D) {
    *out_D = input->count;
    int total_points = 0;
    for (uint32_t d = 0; d < input->count; ++d)
    ...
    *out_sizes = (int *) malloc(input->count * sizeof(int));
    ...
    for (uint32_t d = 0; d < input->count; ++d) {
        ...
        for (uint32_t i = 0; i < input->sizes[d]; ++i) {
        ...
        for (int32_t d = 0; d < D; ++d) {
            ...
            for (int32_t y = 0; y < H; ++y) {
                for (int32_t x = 0; x < W; ++x) {
                    uint32_t flat_index = ((t * D + d) * H + y) * W + x;
