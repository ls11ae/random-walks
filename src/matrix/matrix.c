...
RW_API void matrix_copy_to(Matrix *dest, const Matrix *src) {
    ...
    memcpy(dest->points, src->points, dest->width * dest->height * sizeof(double));
}
...
RW_API Matrix *matrix_add(const Matrix *a, const Matrix *b) {
    ...
    const double *data_a = a->points;
    const double *data_b = b->points;
    double *data_result = result->points;
...
RW_API Matrix *matrix_invert(const Matrix *input) {
    ...
    if (input->width == 2) {
        ...
        inv->points[0] = input->points[3] / det;
        inv->points[1] = -input->points[1] / det;
        inv->points[2] = -input->points[2] / det;
        inv->points[3] = input->points[0] / det;
...
RW_API double matrix_determinant(const Matrix *mat) {
    ...
    if (mat->width == 2) {
        return mat->points[0] * mat->points[3] - mat->points[1] * mat->points[2];
    }
...
RW_API Matrix *matrix_combind(const Matrix *matrix1, const Matrix *matrix2) {
    ...
    for (int i = 0; i < matrix1->len; i++) {
        result->points[i] = matrix1->points[i] * matrix2->points[i];
    }
    return result;
}
...
RW_API Matrix *matrix_upsample_bilinear(const Matrix *input, ssize_t new_w, ssize_t new_h) {
    ...
    for (size_t ny = 0; ny < new_h; ny++) {
        for (size_t nx = 0; nx < new_w; nx++) {
            ...
            double A = input->points[y * input->width + x];
            double B = input->points[y * input->width + x1];
            double C = input->points[y1 * input->width + x];
            double D = input->points[y1 * input->width + x1];

            output->points[ny * new_w + nx] =
                    A * (1 - x_diff) * (1 - y_diff) +
                    B * x_diff * (1 - y_diff) +
                    C * (1 - x_diff) * y_diff +
                    D * x_diff * y_diff;
...
RW_API Matrix *matrix_rotate(Matrix *original, double angle) {
    ...
    rotated->points = (double *) malloc(rotated->len * sizeof(double));
    ...
    for (size_t i = 0; i < original->height; i++) {
        for (size_t j = 0; j < original->width; j++) {
            ...
            if (new_i < new_height && new_j < new_width) {
                rotated->points[new_i * new_width + new_j] = original->points[i * original->width + j];
            }
        }
    }
    ...
    for (size_t y = 0; y < new_height; y++) {
        for (size_t x = 0; x < new_width; x++) {
            ...
            if (new_x < original->width && new_y < original->height) {
                rotated->points[y * new_width + x] = original->points[new_y * original->width + new_x];
            }
        }
    }
