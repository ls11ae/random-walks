...
Matrix *generate_length_kernel_ss(const ssize_t size, const double scaling) {
	ssize_t subsampling = 1;
	const ssize_t kernel_size = size;
...
Tensor *generate_correlated_kernels(const ssize_t dirs, const ssize_t size,
	...
	Matrix *length_kernel = generate_length_kernel_ss(size, length_diffusity);
	Matrix *angle_kernel = generate_angle_kernel_ss(size, angle_diffusity);
	...
	for (int i = 0; i < dirs; ++i) {
		...
		rotate_kernel(rotated_kernel, deg);
...
Tensor *generate_kernel_from_set(const KernelParameters *p, int terrain_value,
	...
	if (result->len != p->D || result->data[0]->width != 2 * p->S + 1) {
		result = generate_correlated_kernels(p->D, 2 * p->S + 1, p->sigma_angle, p->sigma_length);
		return result;
	}
	assert(result);
	if (return_copy) {
