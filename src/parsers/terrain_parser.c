...
TensorSet *generate_correlated_tensors() {
    ...
    for (int i = 0; i < terrain_count; i++) {
        ...
        const int32_t t_D = parameters->D;
        const int32_t M = parameters->S * 2 + 1;
...
KernelsMap *kernels_map_new(const TerrainMap *terrain, const Matrix *kernel) {
    ...
    const int32_t kernel_size = kernel->width;
    ...
    for (int32_t y = 0; y < terrain_height; y++) {
        ...
        for (int32_t x = 0; x < terrain_width; x++) {
            ...
            if (entry) {
                ...
                // Fallback
                fprintf(stderr, "Critical cache miss at (%d, %d)\n", x, y);
                exit(EXIT_FAILURE);
            }
        }
        free(hash_grid[y]);
...
Tensor *generate_tensor(const KernelParameters *p, int terrain_value, bool full_bias,
    ...
    const uint32_t M = p->S * 2 + 1;
    if (p->is_brownian) {
        float scale, sigma;
        get_gaussian_parameters(p->diffusity, terrain_value, &sigma, &scale);
        Matrix *kernel;
        if (full_bias)
            kernel = matrix_generator_gaussian_pdf(M, M, (float) sigma, (float) scale, p->bias_x, p->bias_y);
        else
            kernel = matrix_gaussian_pdf_alpha(M, M, (float) sigma, (float) scale, p->bias_x, p->bias_y);
...
void kernels_map_free(KernelsMap *kernels_map) {
    ...
    for (int32_t y = 0; y < kernels_map->height; y++) {
        for (int32_t x = 0; x < kernels_map->width; x++) {
...
void tensor_map_free(KernelsMap **tensor_map, const uint32_t D) {
...
Tensor *tensor_at(const char *output_path, int32_t x, int32_t y) {
    char path[256];
    snprintf(path, sizeof(path), "%s/tensors/y%d/x%d.tensor", output_path, y, x);
